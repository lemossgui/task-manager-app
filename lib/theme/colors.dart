import 'package:flutter/material.dart';

const Color backgroundColor = Color(0xfff5f5f6);
const Color backgroundColorLight = Color(0xffffffff);
const Color backgroundColorDark = Color(0xffe0e2e1);

const Color primaryColor = Color(0xff00bcd4);
const Color primaryColorLight = Color(0xff62eeff);
const Color primaryColorDark = Color(0xff008ba2);

const Color secondaryColor = Color(0xff424242);
const Color secondaryColorLight = Color(0xff6d6d6d);
const Color secondaryColorDark = Color(0xff1b1b1b);

const Color primaryTextColor = Color(0xff424242);
const Color secondaryTextColor = Color(0xffffffff);

const Color successColor = Colors.green;
const Color infoColor = Colors.blue;
const Color warningColor = Colors.amber;
const Color errorColor = Colors.redAccent;
const Color disableColor = Colors.grey;
